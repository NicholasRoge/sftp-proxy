const {PACKET_MAX_SIZE, PacketType, AttributeFlag, StatusCode} = require('./constants.js')
...
class PacketDecoder {
    ...
    peekUInt64() {
        const highBytes = this.buffer.readUInt32BE(this.offset)
        const lowBytes = this.buffer.readUInt32BE(this.offset + 4)

        if (highBytes !== 0) {
            throw new SftpStatusError(StatusCode.SSH_FX_OP_UNSUPPORTED, "64-bit numbers are currently unsupported.")
        }
...
class PacketEncoder {
    ...
    writeUInt64(value) {
        // Note: Javascript does not support 64-bit integers, so for now just
        // write 0 into the high bytes and pray that the client doesn't try to
        // write a 64-bit number in the mean time
        this._offset = this.buffer.writeUInt32BE(0, this._offset) 
